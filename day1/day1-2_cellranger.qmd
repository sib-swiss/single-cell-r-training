...
## R
```{r day1-2-cellranger-1, eval=FALSE}
download.file(
  url = "https://single-cell-transcriptomics.s3.eu-central-1.amazonaws.com/cellranger_index.tar.gz",
  destfile = "cellranger_index.tar.gz"
)

untar("cellranger_index.tar.gz")

file.remove("cellranger_index.tar.gz")
```
...
We will first download the cellranger

::: {.callout-important}
In case the download link is not working, please get a new one from [here](https://www.10xgenomics.com/support/software/cell-ranger/downloads/previous-versions)
:::


```{r day1-2-cellranger-2, eval=FALSE}
download.file(url = "https://cf.10xgenomics.com/releases/cell-exp/cellranger-9.0.1.tar.gz?Expires=1773802367&Key-Pair-Id=APKAI7S6A5RYOXBWRPDA&Signature=YibknQvib9hYyNL55aAbvsZ8KL1wL74Xduv6uYOyeEArlK8UFSjj23GtyC1WJZ6AgIczTbMZBmxXrs5qlk5HrqkHgu9HIMAErg7EPDVPp49~7A-7SEDopQN2ddBfHBqRoBhmsdNwiOVP5SMq3j3bEKBVWOMoY5kmpX5RteI0nkH1M5-waRZkhU2YWbKCT8mNJGuvPEKXroYWce8b6n7KseWXhwyXXn7MU~IxljGNVuNNeE0MIeqKzV8QAdhW59myhLIBX3Uq9d8HSKTAvi4GBAf7r2GAA7Pa824Et9mTP~d-6sM0otX~XRFNyPsu8oLBePHtI36W6HIaIIY~CBvQnA_",
              destfile = "cellranger-9.0.1.tar.gz")

untar("cellranger-9.0.1.tar.gz")

file.remove("cellranger-9.0.1.tar.gz")
```
...
- pre-indexed reference: `/home/rstudio/single_cell_course/cellranger_index`
...
```{r day1-2-cellranger-3, eval=FALSE}
system(command = "cellranger-9.0.1/cellranger count --id=FIXME --sample=FIXME  --transcriptome=FIXME --fastqs=FIXME --localcores=4  --create-bam=true")
```

### Terminal

```sh
cellranger-9.0.1/cellranger count \
--id=FIXME \
--sample=FIXME \
--transcriptome=FIXME \
--fastqs=FIXME \
...
```{r day1-2-cellranger-4, eval=FALSE}
system(command = "cellranger-9.0.1/cellranger count --id=ETV6-RUNX1_1 --sample=ETV6-RUNX1_1  --transcriptome=cellranger_index --fastqs=course_data/reads --localcores=4  --create-bam=true")
```
...
::: {.callout-important}
As we are not running this advanced exercise for the course, you would be looking at previously generated report: [report_file](../assets/web_summary.html)
:::
...
What should worry us more is the **number of reads per cell** (~350) and the **sequencing saturation** (~7%). In most cases you should aim for 30.000 - 50.000 reads per cell (depending on the application). We therefore don't have enough reads per cell. However, as you might remember, this was a subset of reads (1 million) mapped against chromosome 21 & 22, while the original dataset contains 210,987,037 reads. You can check out the original report at `course_data/count_matrices/ETV6-RUNX1_1/outs/web_summary.html`.
    
For more info on sequencing saturation, have a look [here](https://kb.10xgenomics.com/s/article/115002474263-How-much-sequencing-saturation-should-I-aim-for). 
:::


Clear your environment:

```{r day1-2-cellranger-5}
#| eval: false
gc()
```
