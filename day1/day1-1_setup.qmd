...
{{< downloadthis ../assets/pdf/02_sc_workflow.pdf dname="02_sc_workflow" label="Download the presentation" icon="filetype-pdf" >}}
...
-e PASSWORD=test \
-v $PWD:/home/rstudio \
sibswiss/training-singlecell-rstudio:2026.3
```
...
Now that we have setup a project and a project directory (it is in `/home/rstudio/single_cell_course`), we can download the data that is required for this course.


### To download and extract the dataset, copy-paste these commands in the **Console** tab:

```{r day1-1-setup-4, eval=TRUE}
download.file(
  url = "https://single-cell-transcriptomics.s3.eu-central-1.amazonaws.com/course_data.tar.gz",
  destfile = "course_data.tar.gz"
)
untar("course_data.tar.gz")

file.remove("course_data.tar.gz")
```
